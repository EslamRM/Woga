<template>
    <div class="flex flex-col lg:flex-row gap-24 items-center justify-around my-32 py-11 relative">
        ...
        <div class="flex flex-col justify-center items-start">
            <div class="mb-5">
                <p class="text-4xl font-semibold mb-2">{{ $t("welecome") }}</p>
            </div>
            <SvgTitleSvg />
        </div>
        <form action="" method="post" class="flex flex-col items-start justify-center gap-3 w-[90%] md:w-1/3">
            <div class="w-full">
                <label class="text-sm flex justify-start items-center gap-2">
                    <svg id="Profile" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 20.792 25.999">
            ...
            <div class="w-full">
                ...
                <input type='text' v-model="last_name" @input="lastname_valid = false"
                    class="w-full rounded-lg py-2.5 px-4 mt-2 text-sm outline-none border border-gray-400 bg-white" />
                <span v-if="lastname_valid"
                    class="mt-2 text-sm text-red-500 peer-[&:not(:placeholder-shown):not(:focus):invalid]:block">
                    {{ lastname_valid }}
                </span>
            </div>
            <div class="w-full">
                ...
                <input type='email' v-model="email" @input="email_valid = false"
                    class="w-full rounded-lg py-2.5 px-4 mt-2 text-sm outline-none border border-gray-400 bg-white" />
            ...
            <div class="w-full">
                ...
                <input type='password' v-model="password" @input="password1_valid = false"
                    class="w-full rounded-lg py-2.5 px-4 mt-2 text-sm outline-none border border-gray-400 bg-white" />
            ...
            <div class="w-full">
                ...
                <input type='password' v-model="password2" @input="password2_valid = false"
                    class="w-full rounded-lg py-2.5 px-4 mt-2 text-sm outline-none border border-gray-400 bg-white" />
                <span v-if="password2_valid"
                    class="mt-2 text-sm text-red-500 peer-[&:not(:placeholder-shown):not(:focus):invalid]:block">
                    {{ password2_valid }}
                </span>
            </div>
            <button @click.prevent="register" class="mt-8 text-white bg-secondary rounded-full text-sm px-6 py-3">
                {{ $t('register') }}
            </button>
        </form>
...
export default {
    ...
    methods: {
        ...
        showSuccess() {
            this.showNotify("Success", "Operation completed successfully", "success");
        },
        showError() {
            this.showNotify("Error", this.non_field_errors ? this.non_field_errors : "An error occurred", "error");
        },
        async login() {
        ...
        async register() {
            ...
            await register(formdata, this.showSuccess, this.showError)
        },
    }
}
</script>